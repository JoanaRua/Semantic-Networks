...
PridePrejudice <- readtext::readtext ("~/Desktop/MATLIT/Tese/vizualizacoes/RStudio/SenseandSensibility/SenseandSensibility.txt*") 
PridePrejudice
...
toks <- PridePrejudice_corpus %>%
  ...
  tokens_tolower() %>%
  tokens_remove(pattern =  c(stopwords("en"), "can", "???", "though", "without", "94", "therefore", "yet", "whether", "said", "say", "use", "since", "may", "two",  "still", "upon", 'great', "every", "even", "one?", "must", "?", "one", "us","s"), padding = FALSE)
fcmat <- fcm(toks, context = "window", tri = FALSE)
feat <- names(topfeatures(fcmat, 25))
