...
PridePrejudice <- readtext::readtext ("~/Desktop/MATLIT/Tese/vizualizacoes/RStudio/MansfieldPark/MansfieldPark.txt*") 
PridePrejudice
...
toks <- PridePrejudice_corpus %>%
  ...
  tokens_tolower() %>%
  tokens_remove(pattern =  c(stopwords("en"), "can", "???", "94", "though", "without", "therefore", "yet", "whether", "said", "say", "use", "since", "may", "two",  "still", "upon", 'great', "every", "even", "one?", "must", "?", "one", "us","s"), padding = FALSE)
